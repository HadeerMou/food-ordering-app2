...
const STATUSES = ['confirmed', 'preparing', 'on_the_way', 'delivered', 'cancelled'];

app.use(cors({ origin: allowedOrigin }));
app.use(express.json({ limit: '1mb' }));

const publicUser = (user) => ({ id: user.id, name: user.name, email: user.email, role: user.role, phone: user.phone || '' });
...
function validateProduct(product) {
  if (!product.name || !product.nameAr || !product.description || !product.descriptionAr || !product.category || !product.image) return 'Complete all bilingual product fields and image URL.';
  if (!Number.isFinite(product.price) || product.price <= 0) return 'Price must be greater than zero.';
  return null;
}

app.get('/api/health', (_req, res) => res.json({ ok: true, service: 'nour-api' }));
...
app.post('/api/auth/register', async (req, res, next) => {
  try {
    ...
    if (db.users.some((user) => user.email === normalizedEmail)) return res.status(409).json({ message: 'Email already registered.' });
    const user = { id: `usr_${uuid()}`, name: name.trim(), email: normalizedEmail, password: await bcrypt.hash(password, 10), role: 'user', phone: phone.trim() };
    db.users.push(user);
    await writeDb(db);
...
app.get('/api/menu/categories', async (_req, res, next) => {
  try {
    const db = await readDb();
    const categories = [...new Set(db.products.filter((product) => product.active).map((product) => product.category))];
...
app.get('/api/menu', async (req, res, next) => {
  try {
    ...
    const products = db.products.filter((product) => {
      ...
      const searchable = `${product.name} ${product.nameAr} ${product.description} ${product.descriptionAr}`.toLowerCase();
...
app.patch('/api/menu/:id', requireAuth, requireAdmin, async (req, res, next) => {
  try {
    const db = await readDb();
    const product = db.products.find((entry) => entry.id === req.params.id);
    if (!product) return res.status(404).json({ message: 'Product not found.' });
    const nextProduct = { ...product, ...productInput({ ...product, ...req.body }) };
...
app.delete('/api/menu/:id', requireAuth, requireAdmin, async (req, res, next) => {
  try {
    const db = await readDb();
...
app.post('/api/orders', requireAuth, async (req, res, next) => {
  try {
    ...
    if (!String(address || '').trim()) return res.status(400).json({ message: 'Delivery address is required.' });
    if (!['cod', 'online'].includes(paymentMethod)) return res.status(400).json({ message: 'Choose a valid payment method.' });
    ...
    const orderItems = items.map((entry) => {
      const product = db.products.find((candidate) => candidate.id === entry.productId && candidate.active);
      if (!product) throw Object.assign(new Error(`Product ${entry.productId} is unavailable.`), { status: 400 });
      const quantity = Number(entry.quantity);
      if (!Number.isInteger(quantity) || quantity < 1 || quantity > 20) throw Object.assign(new Error('Each quantity must be between 1 and 20.'), { status: 400 });
      return { productId: product.id, name: product.name, nameAr: product.nameAr, price: product.price, quantity };
    });
    ...
    const order = {
      ...
      transactionId: paymentMethod === 'online' ? `PAY-DEMO-${uuid().slice(0, 8).toUpperCase()}` : null,
      status: 'confirmed',
      createdAt: new Date().toISOString()
    };
    db.orders.unshift(order);
    await writeDb(db);
    return res.status(201).json({ order, message: paymentMethod === 'online' ? 'Mock payment approved.' : 'Order placed. Pay cash on delivery.' });
  } catch (error) { next(error); }
});
...
app.get('/api/orders/:id', async (req, res, next) => {
  try {
    const db = await readDb();
    const order = db.orders.find((entry) => entry.id.toLowerCase() === req.params.id.toLowerCase());
...
app.get('/api/orders/me', requireAuth, async (req, res, next) => {
  try {
    const db = await readDb();
    return res.json({ orders: db.orders.filter((order) => order.userId === req.user.sub) });
  } catch (error) { next(error); }
});
  ...
  try {
    ...
    return res.json({
      stats: {
        ...
        revenue: db.orders.filter((order) => order.paymentStatus === 'paid' || order.paymentMethod === 'cod').reduce((sum, order) => sum + order.total, 0),
...
app.listen(port, () => console.log(`Nour API is running at http://localhost:${port}`));
